...
function getPersonalDashboardData(staffId) {
  ...
  if (staffSheet) {
    ...
    const idIdx = staffHeaders.indexOf('担当者ID');
    const lastNameIdx = staffHeaders.indexOf('苗字（日本語）');
    const firstNameIdx = staffHeaders.indexOf('名前（日本語）');
    const fullNameIdx = staffHeaders.indexOf('氏名（日本語）');
