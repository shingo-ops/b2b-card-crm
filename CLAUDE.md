...
| **A.J.D Framework** | 2環境システム（本番+テスト@HEAD）- 言葉で伝えるだけで自動操縦する開発の相棒 | Claude Code | **常時参照** |
...
| **Section 12** | 【廃止】GAS複数環境フォルダ同期ルール | - | 2環境移行により廃止 |
...
## A.J.D Framework（共通ルール・厳守）

### A.J.D とは

**A.J.D = Autopilot JustSay DevBuddy**

> 「言葉で伝えるだけで自動操縦する開発の相棒」

| 要素 | 意味 | 役割 |
|------|------|------|
| **A**utopilot | 自動操縦 | AIが自律的に開発 |
| **J**ustSay | 言うだけ | 知識不要、言葉で指示 |
| **D**evBuddy | 開発の相棒 | Human-AI協調 |

**対象**: プログラミング知識がなくても、言葉で伝えるだけでシステム開発ができる

### 2環境システム（A.J.D標準）

**全プロジェクト共通**: 2環境（本番 + テスト@HEAD）で運用する。3環境は廃止。

| 環境 | 用途 | 承認 |
|------|------|------|
| **本番** | 実運用（固定デプロイID） | **Human承認必須** |
| **テスト(@HEAD)** | 動作確認（clasp push即反映） | AI単独OK |
...
[JustSay] 言葉で指示
    ↓
[Autopilot] AIが自律的にコード修正 → テスト環境に反映
    ↓
[DevBuddy] Human確認 → 承認 → 本番デプロイ
```

1. **JustSay**: 「〇〇を作って」「△△を直して」と言葉で伝える
2. **Autopilot**: AIがコード修正 → `clasp push` → テスト(@HEAD)で動作確認
3. **DevBuddy**: Humanが確認・承認 → `clasp deploy` → 本番反映

### 禁止事項

| 禁止 | 理由 |
|------|------|
| 3環境以上の構成 | 管理コスト増大、同期漏れリスク |
| Human承認なしの本番デプロイ | 品質担保（DevBuddyの役割） |
| フォルダ分離による環境管理 | 同期作業の廃止 |
...
## 12. 【廃止】GAS複数環境フォルダ同期ルール

> **2026-01-21 廃止**: 3環境（本番/開発/提案）から2環境（本番/テスト@HEAD）に移行。
> フォルダ分離を廃止し、1フォルダ運用に変更。同期作業は不要。
...
| 環境 | フォルダID | 備考 |
|------|-----------|------|
| 本番 | 1JIPeqiT_2ucBUK875sQBcuSYHkT-GtdD | 唯一の運用フォルダ |

> **2026-01-21変更**: 3環境から2環境（本番+テスト@HEAD）に移行。提案/開発フォルダは廃止。
...
> **2026-01-21変更**: 提案環境デプロイIDを廃止（2環境運用に移行）
